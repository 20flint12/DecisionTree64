...
async def key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    """Select an action: Adding parent/child or show data."""
    text = (
        "You may choose to add a family tiytiytytu To abort, simply type /stop."
    )

    buttons = [
        [
            InlineKeyboardButton(text="Kharkiv", callback_data="Kharkiv"),
            InlineKeyboardButton(text="Mragowo", callback_data="Mragowo"),
        ],
        [
            InlineKeyboardButton(text="Show data", callback_data=str(SHOWING)),
            InlineKeyboardButton(text="Done", callback_data=str(END)),
...
def main() -> None:
    ...
    application = Application.builder().token("1773146223:AAHiWcIJn-V5x_qgqOeKyCa1_dZK47vGwi8").build()
